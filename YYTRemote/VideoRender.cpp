...
void VideoRender::paintPic( uint8* pData )
{
	...
	{
		dstWidth = srcWidth/*(int)(srcHeight * winWidth / (float)winHeight)*/;
		dstHeight = srcHeight;
	}
	else//宽不变,高填充黑边
	{
		dstWidth = srcWidth;
		dstHeight = srcHeight/*(int)(srcWidth * winHeight / (float)winWidth)*/;
	}
	...
	{
		m_identifier = YYTPluginManage::getInstance()->getYYTServerHelper()->getUserName().toStdString();
	}
	else
	{
		QMap<QString, RoomMember> roomMembMap = YYTPluginManage::getInstance()->getRoomManage()->getRoomMember();
		if (roomMembMap.contains(QString::fromStdString(m_identifier)))
		{
			m_identifier = roomMembMap.value(QString::fromStdString(m_identifier)).szName.toStdString();
		}
	}
	...
	//TextOutA( hDC, 0, 0, info.toLocal8Bit().data(), info.toLocal8Bit().length());
