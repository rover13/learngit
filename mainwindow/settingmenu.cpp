...
{
    actionNames/*<< tr("Show flyWindow")*/<<FromBits("µÇÂ¼")\
		/* << tr("Show rightBar")\
		<< tr("Check update") << tr("Online help") \
		<< tr("Office site") << tr("About us")*/<< FromBits("ÍË³ö");
}
...
void SettingMenu::initConnect()
{
    ...
    connect(actionMaps[FromBits("µÇÂ¼")], SIGNAL(triggered()), controller, SLOT(showSettingDialog()));
   ...
   // connect(actionMaps[tr("Office site")], SIGNAL(triggered()), controller, SLOT(visitOfficialSite()));
    ...
    connect(actionMaps[FromBits("ÍË³ö")], SIGNAL(triggered()), controller, SLOT(closeMainWindow()));
}

void SettingMenu::setPara(QString strData)
{
	controller->setLoginData(strData);
}
